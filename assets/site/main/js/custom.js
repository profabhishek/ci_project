...
$(document).ready(function () {
    ...
    $('#course_type').on('change', function () {
        ...
        // Reset all rows
        $('.nomenclature').html("<option value=''>Select Nomenclature</option>");
        $('.universty_choice').html("<option value=''>Select University</option>");
        ...
        $.ajax({
            type: "POST",
            url: baseURL + 'applicant/getNomenclatureByType',
            data: frmdata,
            dataType: 'json',
            processData: false,
            contentType: false,
            success: function (data) {
                ...
                var htm = "<option value=''>Select Nomenclature</option>";

                if (data.status === true && data.data.length > 0) {
                    $.each(data.data, function (i, item) {
                        htm += "<option value='" + item.id + "'>" + item.title + "</option>";
                    });
                }

                $('.nomenclature').html(htm);
            }
        });
    });

});
$(document).on('change', '.nomenclature', function () {

    var currentRow = $(this).closest('.courseRow');
    var universityDropdown = currentRow.find('.universty_choice');

    universityDropdown.html("<option value=''>Loading...</option>");

    var courseType = $('#course_type').val();
    var nomenType = $(this).val();

    var frmdata = new FormData();
    frmdata.append('coursetype', courseType);
    frmdata.append('nomen', nomenType);
    frmdata.append(csrfName, csrfHash);

    $.ajax({
        type: "POST",
        url: baseURL + 'applicant/getUniversitybynomenclature',
        data: frmdata,
        dataType: 'json',
        processData: false,
        contentType: false,
        success: function (data) {
            ...
            var htm = "<option value=''>Select University</option>";
	...
	$('.is_accept').change(function () {
		...
		switch (is_accept) {
			...
			case "2":
				...
				$('#university_desg').removeAttr("required");
				$('#university_place').removeAttr("required");
				$('#regional_university').removeAttr("required");
 ...
 BootstrapDialog.confirm('Hi Apple, are you sure?', function(result){
	...
	Dropzone.options.passportPic = {
		success: function (file, response) {
			...
			if (jsresp.status == true) {
				BootstrapDialog.show({ type: BootstrapDialog.TYPE_SUCCESS, title: "Success", message: "Passport Uploaded Successfully.", buttons: [{ label: 'OK', action: function (dialogItself) { dialogItself.close(); $('#psdetails').trigger('click'); } }] });
				$("#passport_image_div").attr("src", jsresp.file_path);
	...
	Dropzone.options.toeflDoc = {
		success: function (file, response) {
			var jsresp = JSON.parse(response);
			if (jsresp.status == true) {
				BootstrapDialog.show({ type: BootstrapDialog.TYPE_SUCCESS, title: "Success", message: "TOEFL Document Uploaded Successfully!", buttons: [{ label: 'OK', action: function (dialogItself) { dialogItself.close(); $('#docsinfo').trigger('click'); } }] });
	...
	Dropzone.options.ieltsDoc = {
		success: function (file, response) {
			var jsresp = JSON.parse(response);
			if (jsresp.status == true) {
				BootstrapDialog.show({ type: BootstrapDialog.TYPE_SUCCESS, title: "Success", message: "IELTS Document Uploaded Successfully!", buttons: [{ label: 'OK', action: function (dialogItself) { dialogItself.close(); $('#docsinfo').trigger('click'); } }] });
	...
	Dropzone.options.duolingoDoc = {
		success: function (file, response) {
			var jsresp = JSON.parse(response);
			if (jsresp.status == true) {
				BootstrapDialog.show({ type: BootstrapDialog.TYPE_SUCCESS, title: "Success", message: "DUOLINGO Document Uploaded Successfully!", buttons: [{ label: 'OK', action: function (dialogItself) { dialogItself.close(); $('#docsinfo').trigger('click'); } }] });
	...
	Dropzone.options.subjectDoc = {
		success: function (file, response) {
			var jsresp = JSON.parse(response);
			if (jsresp.status == true) {
				BootstrapDialog.show({ type: BootstrapDialog.TYPE_SUCCESS, title: "Success", message: "English as a Subject in School/College Document Uploaded Successfully!", buttons: [{ label: 'OK', action: function (dialogItself) { dialogItself.close(); $('#docsinfo').trigger('click'); } }] });
...
//alert(is_accepct);
	...
	switch (acton) {
		...
		case "process":
			...
			if(is_accepct == 1){
				//$('#process_selection').removeclass("hide");
				$('#process_selection').show();
				$('#university_name').attr("required","true");
				$('#university_desg').attr("required","true");
				$('#university_place').attr("required","true");
				$('#signature').attr("required","true");
				
			}else{			
				//$('#process_selection').addClass("hide");
				$('#university_name').removeAttr("required");
				$('#university_desg').removeAttr("required");
				$('#university_place').removeAttr("required");
				$('#signature').removeAttr("required");
				$('#university_name').hide();
				$('#university_desg').hide();
				$('#university_place').hide();
				$('#signature').hide();
				$('.aex').hide();
			}
			
			if (counts1 == 0 || counts2 == 0 || counts3 == 0 ||  counts6 == 0) { 
...
$("#form-process-personal-info").submit(function (e) {
    e.preventDefault();

    var form = document.getElementById("form-process-personal-info");
...
function validateDob() {
		...
		$.ajax({
			...
			success: function (data) {
				if (data.status == true) {
					...
					BootstrapDialog.show({ type: BootstrapDialog.TYPE_DANGER, title: "Error", message: "Above Age Limit", buttons: [{ label: 'OK', action: function (dialogItself) { dialogItself.close(); } }] });
					//document.getElementById("submit").disabled = true;
	...
	else if(apply_type == 10){
		$.ajax({
			type: "POST",
			url: baseURL + 'home/getAyushDob',
			data: { 'year': year, 'month': month, 'date': date, 'apply_type': apply_type },
			dataType: 'json',
			success: function (data) {
				if (data.status == true) {
	...
	else if(apply_type == 12){
		$.ajax({
			type: "POST",
			url: baseURL + 'home/getDiplomaDob',
			data: { 'year': year, 'month': month, 'date': date, 'apply_type': apply_type },
			dataType: 'json',
			success: function (data) {
				if (data.status == true) {
