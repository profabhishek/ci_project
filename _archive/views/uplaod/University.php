...
class University extends CI_Controller {
    ...
    public function getNewUniversityApplicaitons()
    {
		...
		$result = $this->university_model->getUniversityApplications($vars,$universityId);
		//echo "<pre>";print_r($result);die;
		$totalResult = $this->university_model->getTotalUniversityApplications($vars,$universityId);
		...
		if(!empty($result))
		{
			...
			foreach($result as $r)
			{
				...
				$output[] = '<a style="float:left;margin-right:7px;width:104px;" href="'.site_url().'university/viewApplication/'.$applicationDetails[0]['application_no'].'" class="form-control sbmt">Process</a>';
				$output[] = '<a style="float:left;width:104px;" href="'.site_url().'university/viewfullApplication/'.$applicationDetails[0]['application_no'].'" class="form-control sbmt1">View</a>';
				$response[] = $output;
				$counter++;
			}
		}
	...
	public function dashboard()
	{
		try{
			...
			$data['universityData'] = $this->common_model->getUniversityInfo($universityId);
			//$data['misionData'] = $this->common_model->getMissionInfo($missionId);
			//$data['newApplication'] = count($this->university_model->getUniversityApplications($universityId));
