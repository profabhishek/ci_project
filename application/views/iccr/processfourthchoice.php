...
">
	...
	?>
	  ...
	  <!--<h3 class="text-center" id="h5_cnetr">Applicant's Acceptance Forwarded to Hqrs/RO</h3>-->	 
              	...
              	<table class="table" id="tbl_relation" style="width:100%;">
              		...
              		<tbody>
						...
						<tr>
              				<td>
              					<?php
              					$univ = $this->common_model->getUniversityById($data['regional_university']);
              					echo $univ[0]['name'];
              					?>
              				</td>
              				<td>
              				...
              				<td>
              					<?php
              					$reg = $this->common_model->getRegionById($data['region_one_status']);
              					echo $reg[0]['name'];
              					?>
              				</td>
              				<td>
              					<?php              				
              					echo date('d M Y h:i:s A',$data['region_one_status_date']);
              					?>
              				</td>
              				<td>
              					<a target="_blank" href="<?php echo site_url();?>assets/site/main/university_approval/<?php echo  $data['region_one_doc'];?>" target="_blank">Download</a>
              				</td>
              				<td>
              				<?php if($data['university_is_accept'] == 1)
              					  {
									echo "Confirmed";
								  }
								  elseif($data['university_is_accept'] == 2)
              					  {
									echo "Not-Confirmed";
								  }
              				?>
           ...
           <div id="home" class="tab-pane fade in active">	   
	    <form id="form-expenditure" action="<?php echo site_url();?>headquarter/forwardtohqrs/<?php echo $this->uri->segment(3); ?>" method="post" class="form-horizontal" enctype="multipart/form-data">
	    <div class="box-body">
	    	  <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">				
					
              		<div class="form-group col-xs-9">
					    <label for="inputEmail3" class="col-sm-2">Select University<span class="text-red">*</span></label>
					    <div class="col-sm-8">
						 	...
						 	if($applicaitonStepOne[0]['programme']>0)
						 	{
						 		echo '<optgroup label="State Universities">';	
											...
											foreach($statewiseUniversites as $key=>$univercity1)
											{
												if(count($univercity1)>0)
												{
													...
													foreach($univercity1 as $uni_choice)
													{
														if(!empty($applicaitonStepOne))
													  	{											  		
															if($applicaitonStepOne[0]['universty_choice'] == $uni_choice['id'] || $applicaitonStepOne[0]['universty_choice_two'] ==  $uni_choice['id'] || $applicaitonStepOne[0]['universty_choice_three'] ==  $uni_choice['id'])
															{
											...
											{								
												if(!empty($applicaitonStepOne))
											  	{
													if($applicaitonStepOne[0]['universty_choice'] == $univercity_cnet['id'] || $applicaitonStepOne[0]['universty_choice_two'] ==  $univercity_cnet['id'] || $applicaitonStepOne[0]['universty_choice_three'] ==  $univercity_cnet['id'])
													{
											...
											{								
												if(!empty($applicaitonStepOne))
											  	{
													if($applicaitonStepOne[0]['universty_choice'] == $univercity_nit['id'] || $applicaitonStepOne[0]['universty_choice_two'] ==  $univercity_nit['id'] || $applicaitonStepOne[0]['universty_choice_three'] ==  $univercity_nit['id'])
													{
									...
									{								
										if(!empty($applicaitonStepOne))
									  	{
											if($applicaitonStepOne[0]['universty_choice'] == $univercity_yogs['id'] || $applicaitonStepOne[0]['universty_choice_two'] ==  $univercity_yogs['id'] || $applicaitonStepOne[0]['universty_choice_three'] ==  $univercity_yogs['id'])
											{
					...
					<div class="form-group col-xs-6">
					    <label for="inputEmail3" class="col-sm-3">Upload Letter <span class="text-red">*</span></label>
					    <div class="col-sm-8">
