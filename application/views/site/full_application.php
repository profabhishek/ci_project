...
<section class="meacontent" id="printableArea">
	<div class="col-xs-10 form_head">
		...
		?>
			...
			<h4 class="text-center caps">Application Form For Scholarship through SFS (Self Finance Students)</h4>
		<?php
		} else {
		?>
			<h4 class="text-center caps">Application Form For Scholarship through ICCR</h4>

		<?php
		}
		?>

		<h5 class="text-center" style="">TO BE FILLED BY APPLICANT</h5>
	</div>
...
">
		...
		<div class="tab-content detailpagepdf">
			<div id="home" class="tab-pane fade in active">
				...
				<div class="row">
						<div class="col-12 prfl pull-right mr-5">
						<?php

						//$dir = '/assets/'.$user_data['dir'].'/'. $userImage.'/';
						...
						if ($userImage == "") {
						?>
							<img style="max-width:100%;height:auto;" id="profil_image_div" src="<?php echo site_url(); ?>assets/site/main/images/default_avatar.png" />
							<?php
						} else {
							if ($user_data['dir'] == "") {
							?>
								<img style="width:140px;height:171px;" id="profil_image_div" src="<?php echo site_url(); ?>assets/site/main/profile_pics/<?php echo $userImage; ?>" />
							<?php
							}
							/* if($q=="Empty"){
							?>
							...
							?>
								<img style="width:151px;height:171px;" id="profil_image_div" src="data:<?php if (!empty($mime_type)) {
																											echo $mime_type;
																										} ?>;base64,<?php if (!empty($data)) {
																																									echo $data;
																																								} ?>" />
						<?php
							}
...
function closeImage() {
    document.getElementById("imageModal").style.display = "none";
								...
								<!-- <tr>
									<td style="height:35px;" colspan="2">
										<table id="tblpassport" class="table" style="width: 100%;">
											...
											<tbody>
												<tr>
													<td><?php //if (!empty($applicaitonStepOne)) if ($applicaitonStepOne[0]['passport_issue_date'] != "") echo $applicaitonStepOne[0]['passport_issue_date'];
														//else echo "NA"; ?></td>
								...
								<!-- <tr>
									<td style="font-weight: bold;"><b>17. Knowledge of English</b></td>
								...
								?>
									...
									?>
										...
										<tr>
											...
											<td>
												<?php if (!empty($applicaitonStepTwo)) echo $applicaitonStepTwo[0]['ug_leaving_department_name']; ?>
									...
									?>
										...
										<tr>
											...
											<td>
												<?php if (!empty($applicaitonStepTwo)) echo $applicaitonStepTwo[0]['phd_leaving_board_name']; ?>
											</td>
											<td>
												<?php if (!empty($applicaitonStepTwo)) echo $applicaitonStepTwo[0]['phd_leaving_university']; ?>
											</td>
											<td>
												<?php if (!empty($applicaitonStepTwo)) echo $applicaitonStepTwo[0]['phd_leaving_subjects']; ?>
											</td>
											<td>
							...
							?>
								...
								?>
									...
									<tbody>
										...
										<tr>
    <td colspan="2" style="height: 35px; vertical-align: top; padding: 10px 0;">
        <label>26. Have you travelled or lived in India in the past?</label>&nbsp;&nbsp;
        <?php
        if (!empty($applicaitonStepThree)) {
            $data = $applicaitonStepThree[0];
            
            if ($data['is_travel_or_live_in_india_before'] == 1) {
                echo '<strong>Yes</strong>';
                
                // Construct the details string in one line
                $details = [];
                if (!empty($data['visit_period'])) $details[] = "Period: " . htmlspecialchars($data['visit_period']);
                if (!empty($data['visit_place']))  $details[] = "Place: " . htmlspecialchars($data['visit_place']);
                if (!empty($data['visit_purpose'])) $details[] = "Purpose: " . htmlspecialchars($data['visit_purpose']);
                
                if (!empty($details)) {
                    echo ' — <span style="color: #555; font-style: italic;">(' . implode(', ', $details) . ')</span>';
                }
                
            } elseif ($data['is_travel_or_live_in_india_before'] == 2) {
                echo 'No';
            }
        }
        ?>
										...
										<br/>
<tr>
    <td colspan="2" style="height: 35px; vertical-align: top; padding: 10px 0; color: #000;">
        ...
        if (!empty($applicaitonStepThree)) {
            ...
            if ($data['is_married'] == 1) {
                ...
                if (!empty($data['spouse_name'])) {
                    $spouseDetails[] = "Name: " . htmlspecialchars($data['spouse_name']);
                }
                if (!empty($data['spouse_passport'])) {
                    $spouseDetails[] = "Passport: " . htmlspecialchars($data['spouse_passport']);
                }
                
                // Output each detail on a new line, no brackets, pure black
                if (!empty($spouseDetails)) {
                    echo '<div style="color: #000; margin-top: 8px; padding-left: 20px; line-height: 1.6;">' . implode('<br>', $spouseDetails) . '</div>';
                }
                
            } elseif ($data['is_married'] == 2) {
                echo 'No';
            }
        }
        ?>
										...
										<tr>
											<td style="text-align:right;"><?php
														...
														<a target="_blank" href="<?php echo site_url(); ?>assets/site/main/profile_signature/<?php echo $applicaitonStepThree[0]['signature_doc']; ?>" target="_blank" title="Click to View Signature"><img style="padding:2px;width:150px;height:75px;" src="<?php echo site_url(); ?>assets/site/main/profile_signature/<?php echo $applicaitonStepThree[0]['signature_doc']; ?>" /></a><br />

													<?php
																				} else {
													?>

														<a target="_blank" href="<?php echo site_url(); ?>assets/site/main/profile_signature/<?php echo $applicaitonStepThree[0]['signature_doc']; ?>" target="_blank" title="Click to View Signature"><img style="padding:2px;width:150px;height:75px;" src="data:<?php echo $mime_type; ?>;base64,<?php echo $data; ?>" /></a><br />
...
<?php
									if (count($applicaitonStepOne) > 0 && array_key_exists($doctypes['subjectDoc']['type'], $docsArray )) {
									?>
										<tr>
											<td><?php echo $counter;
												$counter++; ?></td>
											<td><?php echo $doctypes['subjectDoc']['title']; ?></td>
											<td>
												<?php
												if (array_key_exists($doctypes['subjectDoc']['type'], $docsArray)) {
													$upload++;
												?>
													<a target="_blank" href="<?php echo site_url() . 'applicant/downloadDocs/' . base64url_encode($file_path_subject); ?>"><i class="fa fa-file-pdf-o fa-1x text-red"></i> Click to view Document</a>
												<?php
												} else {
												?>
													<a href="#" data-toggle="modal" data-target="#subjectDiv">Upload</a>
												<?php
												}
												?>
