...
?>
    ...
    <tbody>
						<tr>
                            ...
                            <td> 
                                <?php 
								$university = $this->common_model->getconfirmationDataByMission($applicaitonStepOne[0]['application_no']);
...
<?php
                         ...
                         ?>
                            ...
                            <td>
                              ...
                              //echo "<pre>";print_r($mappingData);
                                   if($mappingData[0]['mission_status'] == 1) {
                                        if ($university[1]['university_is_accept'] == 1) {
                                            ?>
                                            <a target="_blank" href="<?php echo site_url(); ?>applicant/confirmationReceivedWithFormat/<?php echo $university[1]['application_id']; ?>/<?php echo $university[1]['regional_university']; ?>" target="_blank"><span class = "label label-success">Download</span></a>
							...
							<td>
                              ...
                              //echo "<pre>";print_r($mappingData);
                                   if($mappingData[0]['mission_status'] == 1 && $mappingData[0]['scholar_acceptance'] == 1) {
                                        if ($university[1]['university_is_accept'] == 1) {
                                            ?>
                                            <a target="_blank" href="<?php echo site_url(); ?>applicant/undertakingFromStudent/<?php echo $university[1]['application_id']; ?>/<?php echo $university[1]['regional_university']; ?>" target="_blank"><span class = "label label-success">Download</span></a>
			...
			elseif(count($applicaitonStepOne) > 0 && $applicaitonStepOne[0]['course_type'] == 2)
				{
...
?>
                    ...
                    <th>University Status</th>	
                    <th>University Letter</th>	
					 <!-------- <th >University Fee Structure</th>---->
					 <th>Offer Letter</th>	
                    <th >Acceptance Letter</th>	
    ...
    ?>
							...
							<td>
							  ...
							  $file_path_un = '../../'.$currentyear.'/university_approval/'.$response[0]['region_one_doc'];
                                            ...
                                            ?>
                                            <a target="_blank" href="<?php echo site_url(); ?>applicant/undertakingFromStudent/<?php echo $university[0]['application_id']; ?>/<?php echo $university[0]['regional_university']; ?>" target="_blank"><span class = "label label-success">Download / Print</span></a>
			...
			?>
					...
					if($applicaitonStepOne[0]['country'] == 1)
					{
						echo '<div></div>';
					}
					else
					...
					if($mappingData[0]['scholarship_id']>0 && $mappingData[0]['undertaking_doc'] != NULL )
						{
							?>
							
							 
                <!-----<h4 class="text-center" id="h5_cnetr">(Download Undertaking Format: <a href="<?php echo site_url(); ?>applicant/undertakingFromStudent/<?php echo $this->uri->segment(3); ?>" >Download</a>)</h4>---->	 
                ...
                <div class="box-body">
                    <div class="passport-sec col-xs-12 col-sm-12 col-md-12 pdleft pdright">
                        <form id="form-visa" action="<?php echo site_url(); ?>applicant/UploadUnderTaking/<?php echo $this->uri->segment(3); ?>" enctype="multipart/form-data" method="post" class="form-horizontal">
                            <div class="box-body">
                                <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>"/>	   	 
								<input type ="hidden" name = 'id' value = "<?php echo $id;?>" />

                                <div class="form-group col-xs-12" style = "padding-left:10px;">
									...
									<div id="phyfit" style="display:none;">	
									<label for="inputEmail3" class="col-sm-2 control-label  pdright pdleft">Medical Fitness Certificate</label>
									<div class="col-sm-3">
                                        <input type="file" name="file" class="form-control">
										<p style="color: red; font-size: 12px; margin-top: 5px;">
    * Please upload file in PDF format only, maximum size 200KB
</p>
                                    </div>
...
<script>
document.getElementById("scholar_is_accept").addEventListener("change", function () {
    var phyfit = document.getElementById("phyfit");
    
    if (this.value === "1") { 
        phyfit.style.display = "block";
    } else {
        phyfit.style.display = "none";
    }
});
</script>
...
<?php
								...
								<!----<div class="form-group col-xs-12">
								 <label for="inputPassword5">English Test</label>
<textarea  id="inputPassword5" class="form-control" aria-describedby="passwordHelpBlock" name = "english_test"></textarea>
<small id="passwordHelpBlock" class="form-text text-muted">
You must be 150-250 characters long, contain letters and numbers, and must not contain spaces, special characters, or emoji.
</small></div></br>--->
