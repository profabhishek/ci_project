...
<section class="meacontent">	
	...
	<form method="post" action="<?php echo base_url();?>headquarter/downloadApplication/<?php echo base64_decode($this->uri->segment(3)); ?>" enctype="multipart/form-data" id="frm_details_ngo" name="frm_details_ngo" style="width: 100%;position: relative;top: -43px;
">
		  <input id="approvedappId" name="approvedappId" value="<?php echo base64_decode($this->uri->segment(3)); ?>" type="hidden"/>
						  ...
						  ?>
							...
							<td colspan="2">
								...
								<td style="min-height:35px;font-weight: bold;">
									<label><b>7. Telephone/Mobile Number</b></label>
								</td>
										...
										if(count($applicaitonStepOne)>0 && $applicaitonStepOne[0]['programme'] == 3 || $applicaitonStepOne[0]['programme'] == 4 || $applicaitonStepOne[0]['programme'] == 8)
					{
						...
						else
						{
							...
							<?php	
						  	...
						  	<td>
						  		<?php
					if(count($applicaitonStepOne)>0 && $applicaitonStepOne[0]['programme'] == 1 || $applicaitonStepOne[0]['programme'] == 2 || $applicaitonStepOne[0]['programme'] == 9 || $applicaitonStepOne[0]['programme'] == 3 || $applicaitonStepOne[0]['programme'] == 4)
					{					
						...
						{
							if(!empty($applicaitonStepOne)) echo $applicaitonStepOne[0]["course_subject"];
						}							
					}
					else
					{						 
              	...
              	<table class="table" id="tbl_relation" style="width:100%;">
              		...
              		<tbody>
			            ...
			            $univesrsityid = $user_data['university'];
              			$data = $this->common_model->isAnyUniversityResponseConfirms($this->uri->segment(3),$univesrsityid);
              			...
              			if(count($data)>0)
              			{
							?>
              				...
              				<td>
              					...
              					$sch = $this->common_model->getSchemeById($schemeId[0]['scholarship_id']);
							...
							$currentyear = date('Y');
							$file_path_un = './'.$currentyear.'/university_approval/'.$data[0]['region_one_doc'];
							
							?>
								 <a target="_blank" href="<?php echo site_url().'university/downloadDocs/'.base64url_encode($file_path_un);?>" target="_blank">Downlaod</a>
							...
							<td>
							<?php
							if($data[0]['reject_reason']){
									echo $data[0]['reject_reason'];
								}
								else
								{
									echo "NA";
						...
						{
	              				foreach($universityData as $unidata)
	              				{
									?>
									<tr>
              				<td>
              					<?php
              					$univ = $this->common_model->getUniversityById($unidata['regional_university']);
              					echo $univ[0]['name'];
              					?>
              				</td>
              				<td>
              					<?php
              					echo $this->common_model->getCourseName($applicaitonStepOne[0]['application_no'],$unidata['regional_university']);
              				...
              				<td>
              					<?php
              					$reg = $this->common_model->getRegionById($unidata['region_one_status']);
              					echo $reg[0]['name'];
              					?>
              				</td>
              				<td>
              					<?php              				
              					echo date('d M Y h:i:s A',$unidata['region_one_status_date']);
              					?>
              				</td>
              				<td>
              				
              					<a target="_blank" href="<?php echo site_url();?>assets/site/main/university_approval/<?php echo  $unidata['region_one_doc'];?>" target="_blank">Download</a>
              				</td>
              				<td>
              				<?php if($unidata['university_is_accept'] == 1)
              					  {
									echo "Confirmed";
								  }
								  elseif($unidata['university_is_accept'] == 2)
              					  {
									echo "Not-Confirmed";
								  }
              				?>
