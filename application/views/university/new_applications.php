...
<section class="meacontent">
	...
	<marquee>Welcome <?php echo $universityData[0]['name'];?> to ICCR Scholarship Portal</marquee>
		<div class="blue-heading col-md-12 ">
		  ...
		  <a href="<?php echo base_url(); ?>/university/getNewUniversityApplicaitonsCsv/<?php echo $this->uri->segment(3); ?>"  class="btn btn-success pull-right" >Download CSV</a>
...
<div class = "table-responsive">
		<table  class="customTable1 table table-striped table-bordered detailpagepdf newApplication ">
			<thead>
				...
				<th class="noExport">View Application</th>
				<th class="noExport">Email to Student(Only for missing documents)</th>
				<th class="noExport">Action </th>
				<th class="noExport">Status </th>
				<th class="noExport">Download </th>
			</thead>
			<tbody>
...
$(document).ready(function(){
    ...
    reprottable = $('.newApplication').DataTable({
        ...
        "processing": true,
        "searching": false,
        "serverSide": false, // client-side paging (we're not changing controller/model)
        "ajax": {
            // safer: build URL with PHP year (avoids JS+PHP concat oddities)
            "url": "<?php echo site_url('university/getNewUniversityApplicaitons/'.$year)?>",
            "type": "POST",
            "data": function (data) {
            ...
            // handle either: plain array OR object with { data: [...] }
            "dataSrc": function(json) {
                if (!json) return [];
                if (Array.isArray(json)) return json;          // server returned plain array
                if (json.data && Array.isArray(json.data)) return json.data; // server returned { data: [...] }
                // defensive: sometimes your controller may return { "recordsTotal":..., "data": [...] }
                // if nothing matched, return empty array to avoid "null" rows
                return [];
            }
        },
    ...
    /* Clear datepicker fields on load */
    $('.input-daterange input').each(function() {
        $(this).datepicker('clearDates');
    });

    /* Client-side date filter (keeps your old behaviour) */
    $.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
        var min = $('#min-date').val();
        var max = $('#max-date').val();
        var createdAt = data[8] || ''; // adjust index if needed

        if (!min || !max) return true;
        return moment(createdAt).isSameOrAfter(min) && moment(createdAt).isSameOrBefore(max);
    });
    ...
    $('#btn-filter').click(function(){ 
        reprottable.clear().draw();
        reprottable.ajax.reload(null,true);
    });

    $('#btn-reset').click(function(){
        $('#form-filter')[0].reset();
        reprottable.ajax.reload(null,false);
    }); 
