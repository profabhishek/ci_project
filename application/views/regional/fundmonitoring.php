...
<section class="meacontent">	
	<div class="col-xs-10 form_head">		
		...
		<h3 class="text-center caps">All Fund Releases & Update Opening Balance</h3>	
	  ...
	  <div id="home" class="tab-pane fade in active">	   
	    ...
	    <div class="box-body">
			...
			<tbody>
				...
				if(count($totalFundbalance)>0)
				{
					foreach($totalFundbalance as $app)
					{
						?>
						<tr>
...
<section class="meacontent">
	<div  class="container" style="min-height:410px;padding-top:15px;">
		...
		<div class="blue-heading col-md-12 ">
			 <h3> Statement of Fund Received from Headquarter</h3>
	...
	<a href="<?php echo site_url();?>regional/dashboard" class="backbtn">
		...
		<table id="tbl_schrls" class="customTable table table-striped table-bordered detailpagepdf">
			<thead>
				...
				<th>Financial Year</th>				
				<th>1st Quarter
				<br/>Fund (Release Date)</th>
				<th>2nd Quarter<br/>Fund (Release Date)</th>
				<th>3rd Quarter<br/>Fund (Release Date)</th>
				<th>4th Quarter<br/>Fund (Release Date)</th>
				<th>Total</th>	
				<th>View Details</th>
			</thead>
			<tbody>
				...
				if(count($totalFund)>0)
				{
					foreach($totalFund as $app)
					{
						...
						<td><?php if($app['First_Quarter'] == "") echo '0';else echo $app['First_Quarter'].' ('.date('d M Y h:i:s',$app['created']).')'; ?></td>
						<td><?php if($app['Second_Quareter'] == "") echo '0';else echo $app['Second_Quareter'].' ('.date('d M Y h:i:s',$app['created']).')';?></td>
						<td><?php if($app['Third_Quareter'] == "") echo '0';else echo $app['Third_Quareter'].' ('.date('d M Y h:i:s',$app['created']).')';?></td>
						<td><?php if($app['Fourth_Quareter'] == "") echo '0';else echo $app['Fourth_Quareter'].' ('.date('d M Y h:i:s',$app['created']).')';?></td>
						<td>
						<?php 
						$total = 0;
						...
						echo $total;
						?></td>	
						<td><a class="form-control sbmt" style="height:35px;width:140px;" href="<?php echo site_url();?>regional/viewExpenditureDetails/<?php echo $app['FY'];?>">View Details</a></td>										
						</tr>
						<?php	
						$counter++;	
					}
