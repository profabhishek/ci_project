...
<section class="meacontent">	
	<div class="col-xs-10 form_head">		
		...
		<h5 class="text-center">FILLED BY REGIONAL OFFICE</h5>
	</div>
					...
					<table id="fbl_main" style="width: 100%;">
						...
						<tr>
							...
							<td colspan="2"><label>:&nbsp;&nbsp;<?php
								$missions = $this->common_model->getMissionsByCountry($alunamiapplication[0]['mission_id']);
							...
							<tr >
								...
								<?php 
									...
									if(!empty($alunamiapplication)) echo $alunamiapplication[0]['fullname'];?></td>
							</tr>
							<tr >
					  ...
					  ?></td>
						  ...
						  ?>
							...
							<tr>
								...
								<td>:&nbsp;&nbsp;<?php if(!empty($alunamiapplication)) echo $alunamiapplication[0]['present_details'];?></td>	
							...
							<tr>
								...
								<td style="height:35px;">:&nbsp;&nbsp;+<?php if(!empty($alunamiapplication)) echo $alunamiapplication[0]['phone'];?>&nbsp;&nbsp;<?php if(!empty($applicaitonStepOne)) echo $applicaitonStepOne[0]['phone'];?></td>
							</tr>					
								<tr>
						  	...
						  	<td>
						  		<?php 
					if($alunamiapplication[0]['programme'] != 3 && $alunamiapplication[0]['programme'] != 4 && $alunamiapplication[0]['programme'] != 7)
					{
				  		$courses = $this->common_model->getCourseByPrgramme($alunamiapplication[0]['programme']);
				  		foreach($courses as $course)
				  		{
							if(!empty($alunamiapplication))
						  	{
						  		if($alunamiapplication[0]['course'] == $course['id'])
						  		{
									echo $course['title'];
								}										
							}									
              		...
              		<tbody>
              			...
              			if(count($alunamiapplication)>0)
              			{
							?>
              				...
              				<td>
              					<?php $schemee = $this->common_model->getSchemeById($alunamiapplication[0]['scholarship_scheme']);
											echo $schemee[0]['scheme_name']; ?>
              				</td>
              				<td>
              					<?php
              					$reg = $this->common_model->getRegionById($alunamiapplication[0]['regional_id']);
              					echo $reg[0]['name'];
              					?>
              				</td>
              				<td>
              					...
              					if(!empty($alunamiapplication))
								{
									...
									echo $alunamiapplication[0]['forwarding_date'];
								}
              					?>
              				</td>
              				<td>
              					<a target="_blank" download href="<?php echo site_url();?>assets/site/main/alumani_university_letter/<?php echo $alunamiapplication[0]['upload_university_letter'];?>" target="_blank">Download</a>
              				</td>
              				<td>
              				<?php if($alunamiapplication[0]['university_response'] == 1)
              					  {
									echo "Confirmed";
								  }
								  elseif($alunamiapplication[0]['university_response'] == 2)
              					  {
									echo "Not-Confirmed";
								  }
              				?>
              		...
              		<tbody>
              			<?php
              				if(count($alunamiapplication)>0)
              				{
								?>
									<tr>
										<td><?php echo $alunamiapplication[0]['date_of_departure'];?></td>
										<td><?php echo $alunamiapplication[0]['arrival_date_by_mission'];?></td>
										<td><?php if($alunamiapplication[0]['travel_plan_document'] == 1) 
										{
										?>
										<a class='link_div' download href="<?php echo site_url();?>assets/site/main/alumani_travel_plan/<?php echo $alunamiapplication[0]['upload_travel_doc'];?>">Download</a>
										<?php	
										}else{
										...
										?></td>									
										<td><?php echo $alunamiapplication[0]['flight_no'];?></td>
										<td><?php if($alunamiapplication[0]['status'] == 14) echo "Arrived"; elseif($alunamiapplication['status'] == -14) echo "Not Arrived" ;?></td>
										<td><?php if($alunamiapplication[0]['status'] == 14) 
										{
											echo $alunamiapplication[0]['date_of_arrival'];
										}
										elseif($alunamiapplication['status'] == -14) 
										{
											echo "NA";
										}	?></td>
										<td></td>
