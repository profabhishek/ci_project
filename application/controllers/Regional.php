...
$html = $content;
	...
	public function new_applicationslive()
	{
		try{
			$user_data = $this->session->userdata('user_data');
			$regionid = $user_data['state'];		
...
<body>
	...
	<p style="text-align: justify;">1)  We are pleased to inform you that you have been provisionally selected to pursue Course <?php echo $course . ' at under ' . $uninmae[0]['name'] . ' ' . $schemename[0]['scheme_name'] . ' for the Academic Year 2026-2027. You are requested to report ' . $regionInfo[0]['name'] . ' University physically along with all original certificate and testimonials latest by '   . $response[0]['date_of_joining'] . ' and also to Regional Office through Email.'; ?></p>
...
die;
  ...
  $html = ob_get_clean();		
	...
	function getNewApplicaitons()
    {
		...
		$counter = $_POST['start'];
		$result = $this->Regional_model->getRegionalApplications($regionid,$vars,$year);
		//echo "<pre>";print_r($result);die;
		$totalResult = $this->Regional_model->getRegionalTotalNewApplication($regionid,$vars,$year);
		$response = array();
		$counter++;
		...
		if(!empty($result))
		{
		    ...
		    //print_r($r);
				...
				$output[] = date('d-m-Y',$date2);
					$output[] = '<a style="float:left;width:104px;" href="'.site_url().'regional/viewfullApplication/'.base64_encode($applicationDetails[0]['application_no']).'" class="form-control sbmt1" target = "_blank">View</a>';
					...
					$output[] = '<a target="_blank" href="'.site_url().'regional/downloadStudent_zip/'.$r['application_no'].'" target="_blank"><span class = "label label-success">Download</span></a>';	
...
function downloadcsv(){
     ...
     header("Content-Type: application/csv;");
        ...
        $header = array("Sr.No.","Applicant No","Applicant Name","Email Id","Country","Programe","Course","University"); 
        fputcsv($file, $header);
        $cnt=1;
		...
		if($applicationDetails[0]['programme'] == 3 || $applicationDetails[0]['programme'] == 4 || $applicationDetails[0]['programme'] == 8)
						{
							$course = $this->common_model->getProgrammeById($applicationDetails[0]['programme']);
							$fullCourse =  $course[0]['name'].' ('.$applicationDetails[0]['course_subject'].')';
						}
						else
						{
						...
						if($r['apply_course_type'] == 11){
						$universityDetails .= '1) '.$uname1;
						}
						else
						{
						$universityDetails .= '1) '.$uname1;
				        $universityDetails .= '2) '.$uname2;
				        $universityDetails .= '3) '.$uname3;
						$universityDetails .= '4) '.$uname4;
						$universityDetails .= '5) '.$uname5;
						}
					   $programme = $this->common_model->getProgrammeById($r['programme']);
	
         $narray=array($cnt,$applicationDetails[0]['application_no'],$applicationDetails[0]['fullname'].' '.$applicationDetails[0]['middlename'].' '.$applicationDetails[0]['familyname'],
		$applicationDetails[0]['email'],$country[0]['country_name'],$programme[0]['name'],$fullCourse,$universityDetails
			 );	 
			 $cnt++;
			fputcsv($file, $narray); 
...
//$exceldata[] = $myarray1;	
	...
	function scholararrival()
	{
		try{
			...
			//$data['travelPlan'] = $this->common_model->getApplicationIdsofTravelPlan($regionid);
			$this->load->view('regional/header_regional');
	...
	public function getBonafide() {
			...
			foreach($result as $r)
			{
				...
				//print_r($bonafide);die;
						...
						$upload = '<a href="'.site_url().'regional/uploadbonafide/'.base64_encode($r['application_no']).'" class="form-control sbmt">Upload</a>';
						if($r['bonafide_doc']){
						...
						{
							$uploadpermit = '<a href="'.site_url().'regional/uploadresidentialpermit/'.base64_encode($r['application_no']).'" class="form-control sbmt">Upload</a>';
						...
						{
							$uploadjoining = '<a href="'.site_url().'regional/uploadjoiningreport/'.base64_encode($r['application_no']).'" class="form-control sbmt">Upload</a>';
	...
	public function getAcademicDetails() {
			...
			foreach($result as $r)
			{
				...
				$uploadaAcdemic = '<a class="form-control sbmt" style="height:35px;width:140px;" href="'.site_url().'regional/addcurrentAcademicDetails/'.base64_encode($r['application_no']).'" class="form-control sbmt" target = "__blank">Update Details</a>';		
			...
			foreach($result as $r)
			{
				...
				$uni = $this->common_model->getSchemeById($r['scholarship_id']);			
						...
						if(count($status) <= 0)
						{
							...
							$uploadpermit = '<a class="form-control sbmt" style="height:35px;width:140px;" href="'.site_url().'regional/regionalExpenditureUser/'.base64_encode($r['application_no']).'" class="form-control sbmt" target = "__blank">Payment Details</a>';
	...
	function alumani()
	{
	 ...
	 public function viewAlumaniDetails() {
        try {
            ...
            $data['misionData'] = $this->common_model->getMissionInfo($missionId);
            $data['alunamiapplication'] = $this->common_model->getRegionalAlumaniApplicationbyId($appno);
            $this->load->view('regional/header_regional');
            $this->load->view('regional/viewAlumaniApplication', $data);
            $this->load->view('regional/footer');
        } catch (Exception $e) {
...
$html = $content;
   ...
   function calcFY($startDate,$endDate) 
   {
		...
		public function getScolarArrival()
    {
		...
		function getNewApplicaitonslive()
    {
...
//$output[] = '';							
			...
			foreach($result as $app)
			{
				...
				if($app['status'] >= 4)
						...
						if(count($array_uni_id)>0)
						{
								foreach($array_uni_id as $optionNo)
								{
									...
									if($optionNo == 1)
									{
						              ...
						              //$hqrsConfirm1= '<a style="height:34px;width:140px;" class="form-control sbmt" target="_blank"  href="'.site_url().'regional/application/'.$app['application_no'].'">Confirm to Hqrs</a>';
									   ...
									   //$hqrsConfirm1= '<a style="height:34px;width:140px;" class="form-control sbmt" target="_blank" class="btn btn-block btn-default sbmt disabled" href="#">Confirm to Hqrs</a>';
									if($app['programme'] == 3 || $app['programme'] == 4){
									 $hqrsConfirm1= '<a style="height:34px;width:140px;" class="form-control sbmt" target="_blank"  href="'.site_url().'regional/application/'.$app['application_no'].'">Confirm to Hqrs</a>';
								...
								{
									$hqrsConfirm2 = '<a style="height:34px;width:140px;" class="form-control sbmt disabled" target="_blank"  href="#">Confirm to Hqrs</a>';
						...
						//$hqrsConfirm2 = '<a style="height:34px;width:140px;" class="form-control sbmt" target="_blank"  href="'.site_url().'regional/application/'.$app['application_no'].'">Confirm to Hqrs</a>';
									...
									if($app['programme'] == 3 || $app['programme'] == 4)
								{
									...
									$hqrsConfirm3= '<a style="height:34px;width:140px;" class="form-control disabled sbmt" target="_blank"  href="#">Confirm to Hqrs</a>';
										  $hqrsConfirm .= $hqrsConfirm3;
								}			
						//$hqrsConfirm3= '<a style="height:34px;width:140px;" class="form-control sbmt" target="_blank"  href="'.site_url().'regional/application/'.$app['application_no'].'">Confirm to Hqrs</a>';
									...
									if($optionNo == 1)
									{
										...
										{	
											...
											{											
												...
												$output[] = '<a style="height:34px;width:140px;" class="form-control sbmt disabled" target="_blank"  href="#">Confirm to Hqrs</a>';
	...
	function get_universityResponseSentByRegionToMission() {
		...
		$vars = $this->input->post();
		$counter = $_POST['start'];
		$result = $this->Regional_model->getUniversityResponseSentByMission($vars,$res,$regionid);
		$totalResult = $this->Regional_model->getTotalUniversityResponseSentByMission($vars,$res,$regionid);
		//$response1 = array();
		$response = array();
		$counter++;
		if(!empty($result))
				...
				if(file_exists('./'.$currentyear.'/university_approval/'.$r['region_one_doc'])){
                   ...
                   $file_path_un = './'.$currentyear.'/university_approval/'.$r['region_one_doc'];  
                           if(strpos($r['region_one_doc'],'.pdf')){
							 $output[] = '<a href="'.site_url().'regional/downloadDocs/'.base64url_encode($file_path_un).'" target = "_blank"><span class = "label label-success">Download </span></a>';   
						   }
                            else {
						$file_path_un = './'.$currentyear.'/university_approval/'.$r['region_one_doc'];		
				   ...
				   $output[] = '<a download="'.rand().time().'" href="data:'.$mime_type.';base64,'.$data.'" target = "_blank"><span class = "label label-success">Download </span></a>'; 
                                ...
                                $output[] = '<a target="_blank" href="'.site_url().'regional/confirmationReceivedWithNewFormat/'.$r['application_no'].'/'.$r['regional_university'].'" target="_blank"><span class = "label label-success">Download</span></a>';

                                           
                                        }
											if ($r['scholar_acceptance'] == 1) {
										// $output[] = '<a target="_blank" href="'.site_url().'regional/undertakingFromStudentlive/'.$r['application_no'].'/'.$r['regional_university'].'" target="_blank"><span class = "label label-success">Download</span></a>'; 
										  $output[] = '<a target="_blank" href="'.site_url().'regional/undertakingFromStudent/'.$r['application_no'].'/'.$r['regional_university'].'" target="_blank"><span class = "label label-success">Download</span></a>';
											}
											else{
												$output[] = 'NA';
											}
...
//Edit Student Academic Details
  ...
  if($_FILES["files"]["name"] != '')
  {
   ...
   for($count = 0;$count<count($_FILES["files"]["name"]);$count++)
   {
	...
	function undertakingFromStudentlive()
	{
		try
		{
			$applicationId = $this->uri->segment(3);
		...
		public function createTravelplan()
	{
		...
		try
		{
			...
			$data['response'] = $this->common_model->getconfirmationDataByMissionStatus($appid);
			...
			if(!empty($_FILES))
						...
						if(!empty($_FILES['travel_plan_doc']['name']))
					{
						...
						{	
							...
							$target_file = 'assets/site/main/travelplan/'.$imgnametr; 
							move_uploaded_file($file_tmp, $target_file);
			...
			function getNewApplicaitonsDemo()
    {
		    ...
		    //print_r($r);
				...
				$country = $this->common_model->getCountryById($applicationDetails[0]['nationality']);
						...
						else
						{
							...
							$fullCourse .= $course1[0]['title'].' '.$applicationDetails[0]['course_option_name_two'].'<br/>';
							$fullCourse .= $course2[0]['title'].' '.$applicationDetails[0]['course_option_name_three'].'<br/>';
							$fullCourse .= $course2[0]['title'].' '.$applicationDetails[0]['course_option_name_fourth'].'<br/>';
							$fullCourse .= $course2[0]['title'].' '.$applicationDetails[0]['course_option_name_fifth'].'<br/>';
							$output[] = $fullCourse;
						}
						$universityDetails = '';
						$uname1 ="";$array_uni_id = array();
						...
						if($applicationDetails[0]['student_type'] == 11)
						{
							$universityDetails .= '1) '.$uname1.'<br/>';
						}
						else{
	 ...
	 function applicantAcceptance() {
        try {
				...
				$data['listofacceptanceAyush'] = $this->common_model->applicantAcceptanceAyushForRo($regionid);
                $this->load->view('regional/header_regional');
		...
		public function viewfullApplication()
	{
		try
		{
			...
			$data['registerData'] = $this->common_model->getUserData($data['applicaitonStepOne'][0]["uid"]);
			//echo "<pre>";print_r($data['registerData']);
			$data['userImage'] = $image;
			$data['missions'] = $this->common_model->getAllMissions();
			$data['univercities'] = $this->common_model->getUnivercities();	
			$data['applicaitonStepTwo'] = $this->common_model->getApplicationStepTwoByAppno($applicationId);
			$data['applicaitonOther'] = $this->common_model->getApplicationOtherDetailsbyAppNo($data['applicaitonStepOne'][0]['uid']);
			//echo "<pre>";print_r($data['applicaitonOther']);die;
			$data['applicaitonStepThree'] = $this->common_model->getApplicationStepThreebyAppNo($applicationId);
			$data['applicaitonDocuments'] = $this->common_model->getApplicationDocumentsbyAppNo($applicationId);	
			$this->load->view('regional/header_regional');
			$this->load->view('regional/viewFullApplication',$data);
			$this->load->view('regional/footer');
		}
		catch(Exception $e)
		{
   ...
   function openUniversityStatuslive()
    {
		//echo "-------------------";die;
		//echo '<pre>';
		//print_r($_POST);die;
  ...
  <?php	
	  ...
	  function applicantAcceptanceStatus() {
        try {
			...
			if($data['applicaitonStepOne'][0]['course_type'] == 1)
			{
	            $data['universityData'] = $this->common_model->getConfirmationofApplicationAyushIds($applicationId);
	...
	public function downloadAluminiApplication()
	{
		try
	    {
	    	$appno = $this->uri->segment(3);
			...
			$mpdf->watermark_font = 'DejaVuSansCondensed';
			$mpdf->showWatermarkText = true;
			$html1 = "<div style='text-align:center;'><img width='300px' src='".site_url()."assets/site/main/images/mea-logo.png'></div><br/><h3 class='caps' style='text-align:center;font-family:verdana;margin-bottom:0;margin-top:0;font-weight:normal;'>Alumini Details</h3><br/>";       	 				
	    	$html1 .= "<div style='font-size:7pt;position:absolute;right:0;width:150px;top:20px;'>Ref. No.".$appno.'<br/>'.date("jS F Y h:i:s")."</div>";
$html = $content; 
			$mpdf->SetDisplayMode('fullpage');			 
			// LOAD a stylesheet
	...
	{	
		...
		//echo "<pre>";print_r($studentsDetails);die;
